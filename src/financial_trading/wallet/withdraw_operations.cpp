                                  ...
                                  Json::Value&     json_result)
{
      ...
      core::BinanceAPI::secret_key_.empty())
  {
    BinanceCPP_logger::write_log(
        "<WithdrawOperations::Withdraw> API key and secret key required");
    return;
  }

  std::string url =
      std::string(BINANCE_HOST) + "/sapi/v1/capital/withdraw/apply";
  ...
  {
    BinanceCPP_logger::write_log(
        "<WithdrawOperations::Withdraw> Failed to get anything.");
  }
}
                                            ...
                                            Json::Value&     json_result)
{
  ...
  {
    BinanceCPP_logger::write_log(
        "<WithdrawOperations::GetWithdrawHistory> API key and secret key "
        "required");
    return;
  }

  std::string url =
      std::string(BINANCE_HOST) + "/sapi/v1/capital/withdraw/history";
  std::string querystring = "";
  ...
  if (!result_json.empty())
  {
    ...
    reader.parse(result_json, json_result);
    BinanceCPP_logger::write_log(
        "<WithdrawOperations::GetWithdrawHistory> Done.");
  }
  else
  {
    BinanceCPP_logger::write_log(
        "<WithdrawOperations::GetWithdrawHistory> Failed to get anything.");
  }
}
