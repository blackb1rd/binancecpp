/*
        ...
        GET /api/v3/ticker/bookTicker - Symbol order book ticker (with symbol
   parameter) Following Google C++ Style Guide naming conventions
*/
...
void CurrentPrices::GetSymbolOrderBookTicker(std::string_view symbol,
  ...
  if (str_result.size() > 0)
  {
    ...
    catch (std::exception& e)
    {
      BinanceCPP_logger::write_log(
          "<CurrentPrices::GetSymbolOrderBookTicker> Error ! %s", e.what());
    }
    BinanceCPP_logger::write_log(
        "<CurrentPrices::GetSymbolOrderBookTicker> Done.");
  }
  else
  {
    BinanceCPP_logger::write_log(
        "<CurrentPrices::GetSymbolOrderBookTicker> Failed to get anything.");
  }
}
