...
void OrderBook::GetOrderBook(std::string_view symbol, int limit, Json::Value& json_result) {
  BinanceCPP_logger::write_log("<OrderBook::GetOrderBook>");

  if (symbol.empty()) {
    BinanceCPP_logger::write_log("<OrderBook::GetOrderBook> Symbol cannot be empty");
    return;
  }
  ...
  if (str_result.size() > 0) {
    try {
      ...
      BinanceCPP_logger::write_log("<OrderBook::GetOrderBook> Error ! %s", e.what());
    }
    BinanceCPP_logger::write_log("<OrderBook::GetOrderBook> Done.");
  } else {
    BinanceCPP_logger::write_log("<OrderBook::GetOrderBook> Failed to get anything.");
  }
}
