...
void AccountInformation::GetAccountTradeList(std::string_view symbol, int limit, long from_id,
  ...
  if (core::BinanceAPI::api_key_.empty() || core::BinanceAPI::secret_key_.empty()) {
    BinanceCPP_logger::write_log("<AccountInformation::GetAccountTradeList> API key and secret key required");
    return;
  }

  if (symbol.empty()) {
    BinanceCPP_logger::write_log("<AccountInformation::GetAccountTradeList> Symbol cannot be empty");
    return;
  }
  ...
  std::string signature = hmac_sha256(core::BinanceAPI::secret_key_.c_str(), query_string.c_str());
  ...
  core::BinanceAPI::CurlAPIWithHeader(url, str_result, extra_http_header, post_data, action);
  ...
  if (str_result.size() > 0) {
    try {
      ...
      BinanceCPP_logger::write_log("<AccountInformation::GetAccountTradeList> Error ! %s", e.what());
    }
    BinanceCPP_logger::write_log("<AccountInformation::GetAccountTradeList> Done.");
  } else {
    BinanceCPP_logger::write_log("<AccountInformation::GetAccountTradeList> Failed to get anything.");
  }
}
