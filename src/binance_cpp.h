...
// Include all Financial Trading modules following official Binance API structure
...
using ServerTime = financial_trading::spot_trading::general_endpoints::ServerTime;
using ExchangeInfo = financial_trading::spot_trading::general_endpoints::ExchangeInformation;
using CurrentPrices = financial_trading::spot_trading::market_data_endpoints::CurrentPrices;
using OrderBook = financial_trading::spot_trading::market_data_endpoints::OrderBook;
using TradeData = financial_trading::spot_trading::market_data_endpoints::TradeData;
using AccountInfo = financial_trading::spot_trading::account_endpoints::AccountInformation;
using OrderManagement = financial_trading::spot_trading::spot_trading_endpoints::OrderManagement;
using UserDataStream = financial_trading::spot_trading::user_data_stream_endpoints::UserDataStream;

// Wallet operations
using DepositOperations = financial_trading::wallet::deposit::DepositOperations;
using WithdrawOperations = financial_trading::wallet::withdraw::WithdrawOperations;
}  // namespace api
...
class BinanceCPP {
 public:
  ...
  static void get_depth(std::string_view symbol, int limit, Json::Value& json_result) {
    api::OrderBook::GetOrderBook(symbol, limit, json_result);
  }

  static void get_aggTrades(std::string_view symbol, int fromId, time_t startTime, 
                           time_t endTime, int limit, Json::Value& json_result) {
    api::TradeData::GetAggregateTradesList(symbol, fromId, startTime, endTime, limit, json_result);
  }

  static void get_klines(std::string_view symbol, std::string_view interval, int limit,
                        time_t startTime, time_t endTime, Json::Value& json_result) {
    api::TradeData::GetKlinesCandlestickData(symbol, interval, limit, startTime, endTime, json_result);
  }
  ...
  // WAPI functions
  static void withdraw(std::string_view asset, std::string_view address, std::string_view addressTag,
                      double amount, std::string_view name, long recvWindow, Json::Value& json_result) {
    api::WithdrawOperations::Withdraw(asset, address, amount, addressTag, "", name, "", recvWindow, json_result);
  }

  static void get_depositHistory(std::string_view asset, int status, long startTime, long endTime, 
                                long recvWindow, Json::Value& json_result) {
    api::DepositOperations::GetDepositHistory(asset, status, startTime, endTime, 0, 1000, "", recvWindow, json_result);
  }

  static void get_withdrawHistory(std::string_view asset, int status, long startTime, long endTime,
                                 long recvWindow, Json::Value& json_result) {
    api::WithdrawOperations::GetWithdrawHistory(asset, "", status, startTime, endTime, 0, 1000, recvWindow, json_result);
  }

  static void get_depositAddress(std::string_view asset, long recvWindow, Json::Value& json_result) {
    api::DepositOperations::GetDepositAddress(asset, "", recvWindow, json_result);
  }
