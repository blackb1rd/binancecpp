...
size_t BinanceAPI::CurlCallback(void* content, size_t size, size_t nmemb, std::string* buffer) {
  if (buffer == nullptr) {
    return 0;
  }
...
void BinanceAPI::CurlAPIWithHeader(std::string& url, std::string& result_json,
                                  std::vector<std::string>& extra_http_header,
                                  std::string& post_data, std::string& action) {
  BinanceCPP_logger::write_log("<BinanceAPI::CurlAPIWithHeader> url = %s", url.c_str());

  if (!curl_) {
    BinanceCPP_logger::write_log("<BinanceAPI::CurlAPIWithHeader> CURL not initialized");
    return;
  }
  ...
  if (res != CURLE_OK) {
    BinanceCPP_logger::write_log("<BinanceAPI::CurlAPIWithHeader> curl_easy_perform() failed: %s",
