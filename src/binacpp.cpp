...
void BinaCPP::get_myTrades(std::string_view symbol, int limit, long fromId,
  ...
  if (api_key.size() == 0 || secret_key.size() == 0) {
    BinaCPP_logger::write_log(
        "<BinaCPP::get_myTrades> API Key and Secret Key has not been set.");
    return;
  }
  ...
  if (str_result.size() > 0) {
    ...
    BinaCPP_logger::write_log(
        "<BinaCPP::get_myTrades> Failed to get anything.");
  }
...

{
  ...
  if (api_key.size() == 0 || secret_key.size() == 0) {
    BinaCPP_logger::write_log(
        "<BinaCPP::get_allOrders> API Key and Secret Key has not been set.");
    return;
  }
  ...
  if (str_result.size() > 0) {
    try {
      ...
      BinaCPP_logger::write_log("<BinaCPP::get_allOrders> Error ! %s",
                                e.what());
    }
    ...
    BinaCPP_logger::write_log(
        "<BinaCPP::get_allOrders> Failed to get anything.");
  }
...
void BinaCPP::get_order(std::string_view symbol, long orderId,
  ...
  if (api_key.size() == 0 || secret_key.size() == 0) {
    BinaCPP_logger::write_log(
        "<BinaCPP::get_order> API Key and Secret Key has not been set.");
    return;
  }
...
void BinaCPP::keep_userDataStream(std::string_view listenKey) {
  ...
  if (api_key.size() == 0) {
    BinaCPP_logger::write_log(
        "<BinaCPP::keep_userDataStream> API Key has not been set.");
    return;
  }
  ...
  if (str_result.size() > 0) {
    ...
    BinaCPP_logger::write_log(
        "<BinaCPP::keep_userDataStream> Failed to get anything.");
  }
...
void BinaCPP::close_userDataStream(std::string_view listenKey) {
  ...
  if (api_key.size() == 0) {
    BinaCPP_logger::write_log(
        "<BinaCPP::close_userDataStream> API Key has not been set.");
    return;
  }
  ...
  if (str_result.size() > 0) {
    ...
    BinaCPP_logger::write_log(
        "<BinaCPP::close_userDataStream> Failed to get anything.");
  }
...
void BinaCPP::get_depositHistory(std::string_view asset, int status,
  ...
  if (api_key.size() == 0 || secret_key.size() == 0) {
    BinaCPP_logger::write_log(
        "<BinaCPP::get_depostHistory> API Key and Secret Key has not been "
        "set.");
    return;
  }
  ...
  BinaCPP_logger::write_log("<BinaCPP::get_depostHistory> url = |%s|",
                            url.c_str());
  ...
  if (str_result.size() > 0) {
    try {
      ...
      BinaCPP_logger::write_log("<BinaCPP::get_depostHistory> Error ! %s",
                                e.what());
    }
    ...
    BinaCPP_logger::write_log(
        "<BinaCPP::get_depostHistory> Failed to get anything.");
  }
...
void BinaCPP::get_withdrawHistory(std::string_view asset, int status,
  ...
  if (api_key.size() == 0 || secret_key.size() == 0) {
    BinaCPP_logger::write_log(
        "<BinaCPP::get_withdrawHistory> API Key and Secret Key has not been "
        "set.");
    return;
  }
  ...
  BinaCPP_logger::write_log("<BinaCPP::get_withdrawHistory> url = |%s|",
                            url.c_str());
  ...
  if (str_result.size() > 0) {
    try {
      ...
      BinaCPP_logger::write_log("<BinaCPP::get_withdrawHistory> Error ! %s",
                                e.what());
    }
    ...
    BinaCPP_logger::write_log(
        "<BinaCPP::get_withdrawHistory> Failed to get anything.");
  }
...
void BinaCPP::get_depositAddress(std::string_view asset, long recvWindow,
  ...
  if (api_key.size() == 0 || secret_key.size() == 0) {
    BinaCPP_logger::write_log(
        "<BinaCPP::get_depositAddress> API Key and Secret Key has not been "
        "set.");
    return;
  }
  ...
  BinaCPP_logger::write_log("<BinaCPP::get_depositAddress> url = |%s|",
                            url.c_str());
  ...
  if (str_result.size() > 0) {
    try {
      ...
      BinaCPP_logger::write_log("<BinaCPP::get_depositAddress> Error ! %s",
                                e.what());
    }
    ...
    BinaCPP_logger::write_log(
        "<BinaCPP::get_depositAddress> Failed to get anything.");
  }
