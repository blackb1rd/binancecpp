...
int main() {
    ...
    try {
        ...
        std::cout << "❌ Modern API initialization failed: " << e.what() << std::endl;
    }
    ...
    try {
        ...
        std::cout << "❌ Legacy API initialization failed: " << e.what() << std::endl;
    }
    ...
    try {
        ...
        if (!result.empty()) {
            std::cout << "✅ Modern API server time: " << result["serverTime"].asUInt64() << std::endl;
        } else {
            std::cout << "⚠️  Modern API server time returned empty result" << std::endl;
        }
    } catch (const std::exception& e) {
    ...
    try {
        ...
        if (!result.empty()) {
            std::cout << "✅ Legacy API server time: " << result["serverTime"].asUInt64() << std::endl;
        } else {
            std::cout << "⚠️  Legacy API server time returned empty result" << std::endl;
        }
    } catch (const std::exception& e) {
    ...
    try {
        Json::Value result;
        binance_cpp::financial_trading::spot_trading::general_endpoints::TestConnectivity::Ping(result);
        std::cout << "✅ Modern API connectivity test successful" << std::endl;
    } catch (const std::exception& e) {
        std::cout << "❌ Modern API connectivity test failed: " << e.what() << std::endl;
    }
    ...
    try {
        ...
        if (!result.empty()) {
            ...
            std::cout << "⚠️  Modern API exchange info returned empty result" << std::endl;
        }
    } catch (const std::exception& e) {
        std::cout << "❌ Modern API exchange info failed: " << e.what() << std::endl;
    }
    ...
    std::cout << "• Google C++ Style Guide naming conventions applied" << std::endl;
    std::cout << "• Binance API documentation category structure followed" << std::endl;
